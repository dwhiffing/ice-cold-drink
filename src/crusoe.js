  ...
  function generateMap(options) {
    ...
    for (var x = 0; x < width; x++) {
      ...
      for (var y = 0; y < height; y++) {
        var dist = Math.sqrt(
          Math.pow((x - width / 2) / islandW, 2) +
          Math.pow((y - height / 2) / islandH, 2),
        )
        var islandFactor =
          (dist < diagonalHalf ? Math.cos((dist * Math.PI) / 2) + size : -10) *
          4 *
          curve

        var terrainFactor =
          (noise.simplex2(x / scale, y / scale) +
            noise.perlin2(x / scaleHalf, y / scaleHalf)) /
          3
        var lakeFactor =
          islandFactor > 0
            ? noise.perlin2(x / scale2, y / scale2) * lakeSize * islandFactor
            : 0
        ...
        if (n < 0) {
          ...
          var climateFactor =
            ((noise.simplex2(x / scale, y / scale) +
              noise.perlin2(x / scale2, y / scale2)) /
              2 +
              elevation) /
            2 +
            islandFactor / (50 / curve)
          if (climateFactor < 0) {
            climateFactor = 0
          }
